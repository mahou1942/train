<?php
 ...
 if(!isset($_SESSION['userInfo'])) {
   ...
   jump('./train-6-saleslogin.php','請先登錄後查看');
    ...
    $sales_add = "INSERT INTO sales VALUES(NULL, '$account', '$password')";
?>
  ...
  <table class="boomer">
    <?php
      ...
      if(empty($account) || empty($password) || empty($password1) ){
        jump('./train-6-salesuseradd.php', '有數據未填，請重新修改');
      }
      if($password != $password1){
        jump('./train-6-salesuseradd.php', '密碼輸入不一致，請重新修改');
      }
      if(mysqli_affected_rows($sql) > 0){
        jump('./train-6-salesuseradd.php', '輸入的帳號已經存在，請重新輸入');
      }else{
