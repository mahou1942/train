<?php
  ...
  if(empty($account) || empty($store_password)){
    jump('./train-6-storelogin.php', '帳號密碼不能為空！');
  }
  ...
  if(mysqli_affected_rows($sql) == 0){
    jump('./train-6-storelogin.php', '帳號不存在！');
  }
  if($enb[5] == '0'){
    jump('./train-6-storelogin.php', '帳號尚未啟用！');
  }
  if($enb[5] == '2'){
    jump('./train-6-storelogin.php', '帳號已停用！');
  }
  ...
  if($store_password === $true_password){
    ...
    jump('./train-6-store.php', '登入成功，跳轉到店家頁面');
  }else{
    jump('./train-6-storelogin.php', '輸入密碼錯誤');
  }
